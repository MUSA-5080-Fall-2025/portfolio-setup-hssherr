...
recidivism_data %>%
  ...
  summarise(
    ...
    avg_age = names(which.max(table(Age_at_Release))),
    avg_prior_felonies = names(which.max(table(Prior_Arrest_Episodes_Felony)))
  ) %>%
...
cat("Mean Recidivism rate:", round(mean(model_data$recidivism), 3), "\n\n")
